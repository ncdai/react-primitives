...
| `fullSwipeThreshold` | `number`                      | `0.5`   | Fraction of the row width the drag must cross to arm the side's `fullSwipe` action      |
...
| `fullSwipe`    | `boolean` | `false` | Run this action when the row is dragged past `fullSwipeThreshold`. One per side |

| Attribute    | Value                              |
| ------------ | ---------------------------------- |
| `data-armed` | Present while this action is armed |

A full swipe dispatches a real click on the action, so one `onClick` serves
both the tap and the gesture. Past its natural width the strip grows to stay
flush with the content edge. Arming collapses the other actions in the strip and
grows the `fullSwipe` one into the space they give up, so the takeover reads as
one movement.
